...
export function drawObject(
    ...
    if (obj.keys) drawObjectKeys(dimensions, ctx, obj);
    if (obj.itemIds?.length) drawObjectItems(dimensions, ctx, obj, LINE_WIDTH);
}
const KEY_OFFSET_PERCENT = 0.05
function drawObjectKeys(dimensions: MapDimensions, ctx: CanvasRenderingContext2D, obj: MapObjWithPos) {
    ...
    ctx.fillText(obj.keys![0], (obj.pos.x + KEY_OFFSET_PERCENT) * dimensions.tileSize, (obj.pos.y + 0.75) * dimensions.tileSize);
    ctx.textAlign = "right";
    ctx.fillText(obj.keys![1], (obj.pos.x + keyOffetRight) * dimensions.tileSize, (obj.pos.y + 0.75) * dimensions.tileSize);

}
// draws the first item on the object
function drawObjectItems(dimensions: MapDimensions, ctx: CanvasRenderingContext2D, obj: MapObjWithPos, LINE_WIDTH: number) {
